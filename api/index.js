...
mongoose
  .connect(
    "mongodb+srv://nguyencongtuanuit:dongkiemem@cluster0.q8urtie.mongodb.net/",
    {
...
app.get("/verify/:token", async (req, res) => {
  try {
    ...
    const user = await Post.findOne({ verificationToken: token });
    if (!user) {
...
app.post("/users/unfollow", async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ messager: "Error unfollowing user" });
  }
});
...
app.get("/profile/:userId", async (req, res) => {
  try {
    ...
    res.status(500).json({ messager: "Error while getting the profile" });
  }
});
